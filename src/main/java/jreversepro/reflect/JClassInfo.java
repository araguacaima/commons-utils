...
public class JClassInfo implements KeyWords {
    ...
    public void processMethods(boolean getBytecode) {
        ...
        for (int i = 0; i < this.getMethods().size(); i++) {
            ...
            jre = getBytecode
                  ? (JReverseEngineer) new JDisAssembler(method, this.getConstantPool())
                  : (JReverseEngineer) new JDecompiler(method, this.getConstantPool());
            try {
                jre.genCode();
            } catch (Exception ex) {
                ex.printStackTrace();
    ...
    private StringBuffer getHeaders() {
        ...
        init.append("\n// JVM VERSiON: " + majorNumber + "." + minorNumber);
        init.append("\n// SOURCEFILE: " + srcFile);
        return init;
    }
    ...
    private StringBuffer getStringifiedFields() {
        ...
        for (int i = 0; i < memFields.size(); i++) {
            ...
            String datatype = cpInfo.getImportedClasses().
     ...
     *                              engineering file.
     */
    public void reverseEngineer(boolean getBytecode) throws ClassParserException, RevEngineException {
