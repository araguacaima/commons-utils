...
public class JMethod extends JMember implements KeyWords {
    ...
    public Map getAllCatchJExceptions() {
        ...
        for (int i = 0; i < exceptionBlocks.size(); i++) {
            newMap.putAll(((JException) exceptionBlocks.get(i)).excCatchTable);
        }
        return newMap;
    }
    ...
    public String getMethodHeader(boolean includeStackInfo) {
        ...
        StringBuffer sb = new StringBuffer();
        String returnType = mInfoClass.getConstantPool().
                getImportedClasses().
        ...
        try {
            ...
            if (this.getName().compareTo(CLINIT) != 0) {
                ...
                for (int i = 0; i < args.size(); i++) {
                    ...
                    String argType = mInfoClass.getConstantPool().
                            getImportedClasses().
                            getClassName(Helper.getJavaDataType(jvmArgType, false));

                    sb.append(argType);
                    if (symbolTable != null) {
                        sb.append(" " + symbolTable.getName(symIndex++, JSymbolTable.ARG_INDEX));
                    }
    ...
    public String getMethodAsString(boolean getBytecode, boolean includeStackInfo) {
        ...
        StringBuffer sb = new StringBuffer(getMethodHeader(includeStackInfo));

        sb.append(getBytecode
