 ...
 */
public class JMainFrame extends JFrame implements ActionListener, KeyWords, WindowListener, AppConstants {
    ...
    public synchronized void openFile() {
        JCustomFileChooser chooser = new JCustomFileChooser(mCurDir, "Class Files", ".class", "Open File");
     ...
     *                              engineering file.
     */
    private synchronized void reverseEngineer(File aFile) throws ClassParserException, IOException, RevEngineException {
        mClassInfo = mSerializer.loadClass(aFile);
        String code = null;
        mClassInfo.reverseEngineer(true);
        code = mClassInfo.getStringifiedClass(mMbrGen.OnDecompiler.isSelected());
...
//      createTree(mClassParser.getClassInfo() , mCurrentClass);
    ...
    public void saveFile() {
        JCustomFileChooser chooser = new JCustomFileChooser(mCurDir, "Java Source Files", ".java", "Save File");
    ...
    public void viewPool() {
        JCpDialog dlgPool = new JCpDialog(this, mCurrentClass, mClassInfo.getConstantPool());
    ...
    private void initAppState() {
        ...
        try {
            ...
            mPnlEditor.setEditorFont(new Font(pp.getProperty(FONT), Font.PLAIN, JDlgFont.OPTIMUM_SIZE));
            ...
            setLocation(0, 0);
            setSize(800, 550);
            mPnlEditor.setEditorFont(new Font(mPnlEditor.DEFAULT_FONT, Font.PLAIN, JDlgFont.OPTIMUM_SIZE));
            mMbrGen.OnLookFeel.setDefaultLookAndFeel();
            log.error("Failed to load property file");
        } catch (IOException ioe) {
            log.error("Exception while closing a property file ");
        }
    }
    ...
    private void saveProperties() {
        try {
            Properties pp = new Properties();
            pp.setProperty(DECOMPILE_FLAG, new Boolean(mMbrGen.OnDecompiler.isSelected()).toString());
            ...
            pp.setProperty(FONT, mPnlEditor.getEditorFont().getFamily());

            FileOutputStream fos = new FileOutputStream(mPropertyFile);
            pp.store(fos, PROP_HEADING);
            fos.close();
        } catch (Exception _ex) {
            log.error("Failed to write Properties" + mPropertyFile);
    ...
    private void createTree(JClassInfo aClassInfo, String aCurrentClass) {
        ...
        for (i = 0; i < listFields.size(); i++) {
            ...
            String datatype = mImports.getClassName(Helper.getJavaDataType(field.getDatatype(), false));
        ...
        for (i = 0; i < listMethods.size(); i++) {
            ...
            String returnType = mImports.getClassName(Helper.getJavaDataType(method.getReturnType(), false));
    ...
    private StringBuffer writeArgs(List aArgs, JImport aImports) {
        ...
        for (int i = 0; i < aArgs.size(); i++) {
            ...
            String argType = aImports.getClassName(Helper.getJavaDataType((String) aArgs.get(i), false));
