 ...
 */
public class JCmdMain implements AppConstants {

    private static Logger log = LoggerFactory.getLogger(JCmdMain.class);
     ...
     */
    static final String CMD_OPTIONS = "Syntax: jrevpro -i|-da|-dc|-vp <Class File>"
                                      + "\n -i Interactive"
                                      + "\n -da disassemble"
                                      + "\n -dc Decompile"
    ...
    static {
        ...
        sb.append(CMD_DISASSEMBLE + " | " + CMD_DA + " [<OutputFile>] \t" + "Disassemble file\n");

        sb.append(CMD_DECOMPILE + " | " + CMD_DC + " [ <OutputFile> ] \t" + "Decompile file\n");
        sb.append(CMD_VIEWPOOL + " | " + CMD_VP + " [ <ConstantPoolIndex> ] \t" + "View ConstantPool\n");
    ...
    public void listen(JClassInfo infoClass, String inputFile) {
        ...
        try {
            log.info("Type '" + CMD_HELP + "' to get started...");
    ...
    private JClassInfo loadClass(String inputFile) {
        ...
        try {
            ...
            log.error("Failed to load class : " + inputFile);
            log.error(cpex.getMessage());
        } catch (IOException ioex) {
            // Extreme case.
            log.error("Failed to load class : " + inputFile);
        }
     ...
     */
    private boolean process(JClassInfo infoClass, String cmd) {
        if (infoClass == null) {
            return false;
        }
        String result = null;
        ...
        if (size != 0) {
            ...
            if (opcode.equals(CMD_EXIT)) {
                ...
                log.info("Debug mode is " + flag);
                return true;
            } else if (opcode.equals(CMD_DISASSEMBLE) || opcode.equals(CMD_DA)) {
                try {
                    String output = null;
                    if (size > 2) {
                        log.error("disassemble [ <outputfile>] ");
                        return true;
                    } else if (size == 2) {
                ...
                return true;
            } else if (opcode.equals(CMD_DECOMPILE) || opcode.equals(CMD_DC)) {
                try {
                    String output = null;
                    if (size > 2) {
                        log.error("decompile [ <outputfile>] ");
                        return true;
                    } else if (size == 2) {
                        output = (String) tokens.get(1);
                    }
                ...
                return true;
            } else if (opcode.equals(CMD_VIEWPOOL) || opcode.equals(CMD_VP)) {
                String msg = "Syntax : viewpool [ <ConstantPoolIndex No> ] ";
                if (size == 1) {
                    msg = infoClass.getConstantPool().getEntryInfo();
                } else if (size == 2) {
                    try {
                        int lIndex = Integer.parseInt((String) tokens.get(1));
